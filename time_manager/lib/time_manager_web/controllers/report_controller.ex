defmodule TimeManagerWeb.ReportController do
  ...
  defp calculate_hours_grouped_by_day(entries) do
    ...
    # # Function to calculate daily hours worked by a user on a specific date
    # def daily_hours(conn, %{"user_id" => user_id, "date" => date_string}) do
    #     user_id = String.to_integer(user_id)

    #     # Parse the date string into a Date struct
    #     case Date.from_iso8601(date_string) do
    #     {:ok, date} ->
    #         # Fetch all clock-in and clock-out entries for the specified user and date
    #         entries = 
    #         from(te in Clock,
    #             where: te.user_id == ^user_id and fragment("DATE(?)", te.time) == ^date,
    #             order_by: te.time
    #         )
    #         |> Repo.all()

    #         # Calculate hours worked by pairing clock-in and clock-out
    #         hours_worked = calculate_hours_worked(entries)

    #         # Send the response with total hours worked
    #         json(conn, %{
    #         user_id: user_id,
    #         date: date_string,
    ...
    # # Function to calculate hours worked by pairing clock-in and clock-out entries
    # defp calculate_hours_worked(entries) do
    #     entries
    #     |> Enum.chunk_every(2)
    #     |> Enum.reduce(0, fn
    #     [%{time: in_time, status: true}, %{time: out_time, status: false}], acc ->
    #         # Calculate the difference in seconds between clock-in and clock-out
    #         duration_in_seconds = DateTime.diff(out_time, in_time)
