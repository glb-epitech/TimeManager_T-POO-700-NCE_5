defmodule TimeManager.TimeTrackingTest do
  ...
  describe "clocks" do
    ...
    test "create_clock/1 with valid data creates a clock" do
      valid_attrs = %{status: true, time: ~U[2024-10-07 08:29:00Z]}

      assert {:ok, %Clock{} = clock} = TimeTracking.create_clock(valid_attrs)
      assert clock.status == true
      assert clock.time == ~U[2024-10-07 08:29:00Z]
    end
    ...
    test "update_clock/2 with valid data updates the clock" do
      clock = clock_fixture()
      update_attrs = %{status: false, time: ~U[2024-10-08 08:29:00Z]}

      assert {:ok, %Clock{} = clock} = TimeTracking.update_clock(clock, update_attrs)
      assert clock.status == false
      assert clock.time == ~U[2024-10-08 08:29:00Z]
    end
  ...
  describe "workingtimes" do
    ...
    test "create_working_time/1 with valid data creates a working_time" do
      valid_attrs = %{start: ~U[2024-10-07 08:36:00Z], end: ~U[2024-10-07 08:36:00Z]}

      assert {:ok, %WorkingTime{} = working_time} = TimeTracking.create_working_time(valid_attrs)
      assert working_time.start == ~U[2024-10-07 08:36:00Z]
      assert working_time.end == ~U[2024-10-07 08:36:00Z]
    end
    ...
    test "update_working_time/2 with valid data updates the working_time" do
      working_time = working_time_fixture()
      update_attrs = %{start: ~U[2024-10-08 08:36:00Z], end: ~U[2024-10-08 08:36:00Z]}

      assert {:ok, %WorkingTime{} = working_time} = TimeTracking.update_working_time(working_time, update_attrs)
      assert working_time.start == ~U[2024-10-08 08:36:00Z]
      assert working_time.end == ~U[2024-10-08 08:36:00Z]
    end
